...
import {
  getPropertyStatusesByLocation,
  getPropertyStatusesGroupedByLocation,
  PropertyStatusOption
} from '../utils/propertyStatusesByLocation';
...
const PropertyStatusSelect: React.FC<PropertyStatusSelectProps> = ({
  value,
  onChange,
  className = '',
  ...
  // Get property statuses based on tenant location
  const propertyStatuses = getPropertyStatusesByLocation(primaryLocation);
  const groupedStatuses = getPropertyStatusesGroupedByLocation(primaryLocation);
  ...
  if (loading) {
    return (
      <div className={cn(
        ...
        <span className="text-gray-500">Loading property statuses...</span>
      </div>
    );
  }
  ...
  return (
    <SelectPrimitive.Root value={value} onValueChange={onChange}>
      ...
      >
        <SelectPrimitive.Value placeholder="Select status" />
                ...
                <SelectPrimitive.Group key={category}>
                  ...
                  {statuses.map((status) => (
                    ...
                    >
                      ...
                      <SelectPrimitive.ItemText>{status.label}</SelectPrimitive.ItemText>
                    </SelectPrimitive.Item>
                  ))}
